...
export const VoiceProvider = ({ children }: { children: ReactNode }) => {
  ...
  const simulateParticipants = (userName: string) => {
    const mockParticipants: Participant[] = [
      { id: 'bot-1', name: 'Voice Assistant', isMuted: false, isDeafened: false, isSpeaking: false, volume: 100 },
      { id: 'bot-2', name: 'Echo Test Bot', isMuted: false, isDeafened: false, isSpeaking: true, volume: 80 },
    ];
    setParticipants(mockParticipants);
    const speakingInterval = setInterval(() => {
      setParticipants(prev => prev.map(p => ({ ...p, isSpeaking: Math.random() > 0.5 })));
    }, 2000);
  ...
  const setParticipantVolume = (participantId: string, newVolume: number) => {
    setParticipants(prev => prev.map(p => p.id === participantId ? { ...p, volume: newVolume } : p));
  };

  const value: VoiceContextType = {
    isConnected, isConnecting, connectionError, roomId, localUserId, isMuted, isDeafened, volume, participants,
    joinRoom, leaveRoom, toggleMute, toggleDeafen, setVolume, setParticipantVolume,
  };
